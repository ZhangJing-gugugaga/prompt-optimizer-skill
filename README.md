# Prompt Optimizer — Hermes Skill

> 把模糊的"帮我写个东西"变成 AI 真正能执行的专家级提示词。

## 这是什么？

一个给 [Hermes](https://github.com/Nicepkg/hermes) 用的 skill，能把用户随手写的模糊需求，通过四阶段分析→规划→生成→验证，变成结构化、可测试、可复用的高质量提示词。
...
把整个 `prompt-optimizer` 目录放到 Hermes 的 skills 目录下：

```
~/.hermes/skills/productivity/prompt-optimizer/
```

放好后 Hermes 会自动识别。触发词包括：`优化提示词`、`改进prompt`、`提示词太模糊`、`AI输出不稳定`、`自动优化提示词` 等。
...
Hermes 会加载本 skill，走完四阶段流程，返回结构化版本。
...
- **v1** (2026-06-21)：合并 `prompt-optimization` + `prompt-optimizer` + `prompt-optimizer-mcp` 三个 skill 为统一版本。保留完整方法论、10 种模式、4 套模板、MCP 集成文档、实战案例。
